...
if __name__ == '__main__':
    ...
    log_name = foldname + str(datetime.now()).replace(':','-')
    if not os.path.exists(os.path.join(model_dir, log_name)):
    ...
    if not os.path.exists(os.path.join(log_dir, log_name)):
        ...
        print(new_dir)
        os.makedirs(new_dir)
    checkpoint_name = os.path.join(model_dir, log_name, 'weights.{epoch04d}-{val_acc.4f}.hdf5').replace('\\','/').replace(':','-')
    ...
    try:
        ...
        model.fit(trainX, trainY, validation_data=(valX, valY), callbacks=[modelcheckpnt, log_metrics(valX, valY, pat_val), csv_logger, tensbd], batch_size=128, epochs=2)  # might have bugs
        #model.fit(trainX, trainY, validation_data=(valX, valY), batch_size=100, epochs=1)  # might have bugs
